...
- A specification document that reads like an editor wrote it. Crisp, unambiguous, with the implicit cases made explicit.
- Conversations with the agent that read like an editorial back-and-forth — questions, clarifications, "what did you mean by that?" probes.
- A noticeably better hit rate on the agent's first or second attempt at a task, because the request was clearer to begin with.
- A willingness to read the agent's output critically, the way an editor reads a draft, rather than skimming it for structure.
- A tendency to write more comments and documentation than was conventional in the pre-agent era — because the comments are now also part of the brief for the next agent run.
...
- *Specification skill* — the broader capability of which English-major performance is one expression.
- *Prompt engineering* — the early, narrow framing of the same skill.
- *Vibe coding* / *bathrobe coding* — adjacent labor-market phenomena describing how the work itself has changed shape.

---

*Related entries: [Naming](naming.md), [Tool](tool.md), forthcoming Bathrobe coding.*
