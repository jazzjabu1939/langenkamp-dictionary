...
- **The newspaper article** must have a headline. The headline must name one cause. *Apple CEO Steps Down After 15 Years* is a headline. *Six Independent Vectors Converged on Apple in Three Weeks* is a memo, and an editor will not run it.
- **The sell-side analyst note** must have a thesis. *Buy on succession* is a thesis. *We see a six-vector convergence and decline to assign a single driver* is not a thesis; it is grounds for not earning the bonus.
- **The classroom case study** must have a decision and a decision-maker. *Tim Cook decided to retire because of X* is a teachable case. *Tim Cook is unsure why he decided what he decided, and so are we* is not a case; it is an honest description of reality, which is not pedagogically tractable in 75 minutes.
- **The AI chatbot** is trained on the prior three. It produces single-arrow output by default, because the training corpus is single-arrow output. Even when prompted to consider multiple causes, it will tend to compress them into a ranked list with one dominant driver — because the corpus told it that is the shape of an answer.
...
When all four are present, the arrow you are reading missed The Truth. The corrective is the [Sixfold Skyreading](sixfold-skyreading.md) practice: walk the six vectors yourself, name which are lit and which are dark, and resist the urge to assign rank.
...
Cook's willingness to actually leave when the cluster lit up is, in this reading, an unusual act of multi-arrow self-honesty: he saw enough of the cluster to act on it, even though no individual arrow was sufficient. The more common pattern, across institutions and centuries, is the leader who waits for *one decisive arrow* — a clear scandal, a clear failure, a clear successor — that never arrives in single-arrow form, while the cluster builds and the cost compounds. Most institutions are damaged by this kind of leader, not by the kind who steps away under multi-vector pressure.

This is, in its own way, an argument for cultivating multi-arrow self-awareness in the people we train as future leaders. The single-arrow habit is comfortable; it is also the habit of staying too long.

## Trade-offs and warnings

- **Multi-arrow narratives are harder to publish, sell, and teach.** This is the cost of seeing clearly. Most of the world will keep buying single-arrow stories. The convergence-aware reader will be perpetually, mildly annoyed at the *Journal*. Accept it.
- **Don't overcorrect into "everything has a hundred causes."** That collapses into untestability. Three to six vectors, named and tracked, is the working sweet spot — see [Sixfold Skyreading](sixfold-skyreading.md).
- **The fallacy is sometimes inverted.** Some events really do have one dominant cause. The corrective is not to *insist* on convergence everywhere, but to *check* for convergence before accepting the single-arrow framing. Below the three-vector threshold, single-arrow may be honest.
- **The arrow-rider ant is a metaphor, not an epistemology.** Real convergence reading does not require imagining oneself as an ant on each arrow. But the image is durable, and durable images are how durable concepts travel.
