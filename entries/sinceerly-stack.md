...
The pitch is funny on at least three levels. It extrapolates from five cold emails and four replies, so it is marketing anecdote rather than evidence about how chief executives generally write. The Sinceerly product packages the anecdote as a service.

The product page also offers a free tier (three rewrites), a Pro tier at $4.99/month for unlimited rewrites, and — this is the load-bearing detail for the entry that follows — a *Bring Your Own API Key* option that lets the user supply an Anthropic key directly. The recursive stack is therefore explicit: a language model can be used to alter prose generated by the same model family or by a competitor.
...
2. **AI to detect AI writing.** Pangram Labs, GPTZero, Originality.ai, Turnitin's AI-detection module, and competitors form a visible commercial market. Its precise size is not needed for the argument.
3. **AI to remove the AI tells.** Sinceerly is the worked example, but it is one of many humaniser products. What distinguishes it here is the explicit framing and the founder's willingness to lean into the joke.
4. **AI to detect the de-tellers.** Detection firms have publicly described adapting to humanisers, creating the adversarial pattern the entry names. That establishes an active detection-and-evasion loop; it does not show that every vendor can reliably identify every humanised text.

A fifth layer is structurally possible: **AI to remove the de-teller-detection signatures**, or tools designed to alter the output of humanisers again. The entry predicts that each layer can create demand for the next; it does not claim evidence for a particular fifth-layer product.
...
**The stack is adversarial against itself, and each additional layer imposes a cost.** That cost may be money, tokens, time, privacy exposure, or attention. A user who enters at layer one becomes a potential customer for the detection and evasion layers that follow.
...
**The stack creates suppliers at every layer.** Model providers may sell additional inference; detection firms sell classification; humaniser firms sell evasion or stylistic alteration. Not every product uses the same provider or pricing model, but the general incentive remains: one layer's output creates demand for the next layer's service.
...
Opting out at layer zero means: when an email or message or post or essay matters, write it yourself, in your own voice, with the cadence of your own reading voice in your head. Use AI tools where they make sense for the *task* — research, summarisation, copy-editing, brainstorming — but produce the final prose with your own attention on the page. This is the *cooperative-writing* practice named in *[AI Writing](/entries/ai-writing/)* and detailed across this cluster. It is the alternative to the stack.
...
**The product makes the recursive problem unusually visible.** Detection products offer a useful signal in some settings, but their scores can be mistaken for authorship verdicts and some tested systems have produced unequal false-positive rates. Sinceerly, by leaning into the absurdity, exposes the structure of the detection-and-evasion market. The Dictionary recommends opting out of that contest when the writer can do so.
...
The marketing plainly leans into the joke. *Sinceerly* is funny because the name is doing part of the diagnosis the founder did not have to write. It is sincerity-by-other-means, packaged as a subscription. The Dictionary's recommendation, restated for this section: *do not enter the stack*. Write the consequential email with enough attention that its final register is one you will own.

## See also

- *[AI Writing](/entries/ai-writing/)* — the parent hub of this cluster
- *[Zombie Internet](/entries/zombie-internet/)* — the medium the stack feeds
- *[Earned Parallelism](/entries/earned-parallelism/)* — the diagnostic for one of the specific tells the humaniser layer targets
- *[The Olang' Trap](/entries/olang-trap/)* — the structural injustice of the detection-layer economy
- *[The Lazy Median Hypothesis](/entries/lazy-median-hypothesis/)* — why so many writers enter the stack in the first place
